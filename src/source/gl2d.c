...
******************************************************************************/
void glQuadFilled( int x1, int y1, int x2, int y2,
...
void glSpriteOnQuadClipped( int x1, int y1,
							...
							int x3, int y3,
							int x4, int y4,
							int luclip, int lvclip,
							int ruclip, int rvclip,
							int flipmode, const glImage *spr 
						  )
{
	
	int	u1 = spr->u_off + (( flipmode & GL_FLIP_H ) ? spr->width-(1+ruclip) : luclip);
 	int	u2 = spr->u_off + (( flipmode & GL_FLIP_H ) ? luclip 		    	  : spr->width-(1+ruclip));
	int v1 = spr->v_off + (( flipmode & GL_FLIP_V ) ? spr->height-(1+rvclip): lvclip);
 	int v2 = spr->v_off + (( flipmode & GL_FLIP_V ) ? lvclip		    	  : spr->height-(1+rvclip));
	...
	glBegin( GL_QUADS );
		...
		gxTexcoord2i( u1, v1 ); gxVertex3i( x1, y1, g_depth );	
		gxTexcoord2i( u1, v2 ); gxVertex2i( x2, y2 );
		gxTexcoord2i( u2, v2 ); gxVertex2i( x3, y3 );
		gxTexcoord2i( u2, v1 ); gxVertex2i( x4, y4 );
